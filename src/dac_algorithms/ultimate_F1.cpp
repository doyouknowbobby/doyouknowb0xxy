...
GCReport getGCMacroReport(GpioToButtonSets::F1::ButtonSet buttonSet) {
    ...
    // Don't need two Up buttons. The pinky button will be smash attack.
    bool smash      = bs.up;            // original up (right pinky bottom)
    bs.up           = buttonSet.up2;    // up2 (left W in WASD)
    // 4 frame shorthops? No thanks.
    bool shorthop   = bs.x;             // original X (right middle bottom)
    ...
    /* Buttons */
    gcReport.a = bs.a || smash;         // A press, or Smash(up) button activates A
    gcReport.b = bs.b || smash;         // B press, or Smash(up) button activates B
    gcReport.x = bs.x;                  // X press activates X
    gcReport.y = bs.y || shorthop;      // Y press, or Shorthop(X) button activates Y
    gcReport.z = bs.z;
    gcReport.l = bs.l;
    gcReport.r = bs.r;
    gcReport.start = bs.start;
